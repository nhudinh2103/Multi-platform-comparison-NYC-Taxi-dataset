...
AS
  SELECT
      ...
      PARSE_DATETIME('%Y%m', FORMAT('%06d', CAST(t.trip_year as INT64) * 100 + CAST(t.trip_month as INT64))) as partition_id
  FROM 
    `amplified-brook-454012-i1.nyc_taxi.green_taxi_trips_raw` t
    LEFT OUTER JOIN `amplified-brook-454012-i1.nyc_taxi.vendor_lookup` v
      ON (CAST(t.vendor_id AS INT64) = v.vendor_id)
    LEFT OUTER JOIN `amplified-brook-454012-i1.nyc_taxi.trip_type_lookup` tt 
